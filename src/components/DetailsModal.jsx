...
// export default function DetailsModal({ item, view = "front", onClose }) {
//     if (!item) return null;
...
export default function DetailsModal({ item, view = "front", onClose }) {
  ...
  // Backward-compat: prefer item.images, fall back to item.views if present.
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-3">
                <div className="grid grid-cols-2 gap-3">
                  {imageDefs
                    .filter(([k]) => !!imgObj[k])
                    .map(([k, label]) => {
                      const src = imgObj[k] || imgObj.front || "";
                      return (
                        <figure key={`${item._key || item.id}-${k}`} className="relative overflow-hidden rounded-lg border border-slate-800">
                          {src ? (
                            <img
                              src={src}
                              alt={label}
